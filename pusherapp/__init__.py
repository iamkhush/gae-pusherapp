...
class Pusher():
    ...
    class Channel():
        ...
        def trigger(self, event, data={},socket_id =None):
            ...
            appid = self.__pusher.get_app_id()
            ...
            pusher_path = '/apps/%s/channels/%s/events' % (appid, self.__name)
            ...
            if socket_id:
                pusher_qs['socket_id'] = unicode(socket_id)
            ...
            pusher_qs_list = zip(keys, vals)
            
            # Concatenate keys, values
            pusher_qs_joined = '&'.join([key + '=' + value for (key,value) in pusher_qs_list])
